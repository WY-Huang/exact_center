...
{
    // 批量读取文件
    std::string folderPath = "/home/wanyel/contours/halcon_calibration/20230919/rawImgs";
    ...
    for (const auto& entry : fs::directory_iterator(directory)) 
    {
        ...
        if (fs::is_regular_file(entry)) 
        {
            ...
            // 重设置图像大小
            // cv::resize(srcimg, srcimg, cv::Size(1536, 1024));
            // cv::imshow("grayTransImg", grayTransImg);
            // cv::waitKey(0);

            // 重命名图像路径
            fs::path path_temp(filePath);
            std::string stem = "/home/wanyel/contours/halcon_calibration/20230919/binaryImgs/" + path_temp.stem().string() + "_binary.jpg";
            std::cout << stem << std::endl;
            cv::imwrite(stem, binaryImg);
